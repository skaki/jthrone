...
public class DocumentController {
    ...
    public @ResponseBody
    Iterable<Document> getAllUsers() {
        return docRepo.findAll();
    }

    @GetMapping("document/{id}")
    @PreAuthorize("hasAuthority('ADMIN_USER') or hasAuthority('STANDARD_USER')")
    public ResponseEntity<Document> getById(@PathVariable(name="id",value = "id") Long id) {
    ...
    @PreAuthorize("hasAuthority('ADMIN_USER')")
    public ResponseEntity<Void> create(@RequestBody Document document, UriComponentsBuilder builder) {
